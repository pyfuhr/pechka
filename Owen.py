...
class OwenProtocol:  # Класс, реализующий протокол ОВЕН напрямую в python
    ...
    def __str__(self):
        return ('Address: {} Address length: {}\nHash: {} Request: {}\nData size: {} Data: {}\n' + \
                'Frame: {}\nFrame in Raw: {}\nCrc: {} Crc is OK: {}').format(self.address, self.addrLen, self.hash,
                                                                             self.request, self.dataSize,
                                                                             list(self.data), \
                                                                             list(self.frame), self.rawFrame, self.crc,
                                                                             self.crcOk)
    ...
    def unpackIEEE32(self, data, withTime=False,
                     withIndex=False):  # извлекает из данных число с плавающей точкой и время
        dataSize = len(data)
        additionalBytes = 0
        if withTime:
            additionalBytes += 2
        ...
        if dataSize != 4 + additionalBytes:
            raise OwenUnpackError(
                'OwenUnpackError: Wrong size of data ({0}) when IEEE32 unpacking, should be {1}!'.format(dataSize, (
                            4 + additionalBytes)), \
                data)
    ...
    def unpackFloat24(self, data):  # извлекает из данных число с плаваЮщей точкой
        dataSize = len(data)
        if dataSize != 3:
            raise OwenUnpackError('OwenUnpackError: Wrong size of data ({0}) when float24 unpacking!'.format(dataSize),
                                  data)
    ...
    def unpackInt16(self, data):  # извлекает из данных целое число со знаком
        dataSize = len(data)
        if dataSize < 1:
            raise OwenUnpackError('OwenUnpackError: Wrong size of data ({}) when short int unpacking!'.format(dataSize),
                                  data)
        elif dataSize == 1:
    ...
    def unpackUnsignedInt16(self, data):  # извлекает из данных целое число со знаком
        ...
        if dataSize < 1:
            raise OwenUnpackError(
                'OwenUnpackError: Wrong size of data ({}) when unsigned short int unpacking!'.format(dataSize), data)
        elif dataSize == 1:
            data = b'\x00' + data  # дополняем до двух байтов
        # value = ord(self.data[1]) + (ord(self.data[0])<<8 & 0xffff)
        # value = struct.unpack('>H', data[0:2])[0]
    ...
    def unpackChar(self, data):  # извлекает из данных байт со знаком, возвращает целый тип
        dataSize = len(data)
        if dataSize != 1:
            raise OwenUnpackError('OwenUnpackError: Wrong size of data ({0}) when char unpacking!'.format(dataSize),
                                  data)
    ...
    def packFrame(self, hash, address, request,
                  data):  # формирует массив байтов из данных класса для передачи на устройство, возвращает фрейм в виде b строки
        self.frame.clear()
        # frame = bytearray()
        # адрес
        if self.addrLen == 8:
    ...
    def getPingPong(self, address, name, request=True, data=b''):  # отправка фрейма запроса, получение ответа
        ...
        try:
            ...
            self.DebugMessage(
                'Sending::frame size: {0}  addr: {1:#x} hash: {2:#x}'.format(len(self.frame), address, hash))
            if not request:
...
class OwenDevice(OwenProtocol):
    ...
    def getNetworkSettings(self, address=None):  # возвращает сетевые параметры прибора
        baudRate = [2400, 4800, 9600, 14400, 19200, 28800, 38400, 57600, 115200][
            self.getInt16('bps', address)]  # Скорость обмена (бод)
        ...
        return 'Baud rate: {}, Bit length: {}, Parity: {}, Stop bits: {}, ' \
               ...
               'Answer delay: {}'.format(baudRate, bits, parity, stopBits, baseAddress, addressLength, errorNumber,
                                         answerDelay)